...
const _SND_PICKUP = preload("res://assets/audio/armorpickup.ogg")
...
func _ready():
	...
	if not is_connected("body_entered", self, "_on_ArmorPickup_body_entered"):
		connect("body_entered", self, "_on_ArmorPickup_body_entered")
...
func _on_ArmorPickup_body_entered(body):
	...
	if body.has_method("add_armor"):
		var was_added = body.add_armor(armor_amount)
		if was_added:
			pickup()
